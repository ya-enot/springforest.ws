...
package org.filatov.springforest.ws.service.endpoint.mapping.marshaller;
 ...
 *
 * <p>
 * The typical usage will be to set either the "contextPath" or the
 * "classesToBeBound" property on this bean, possibly customize the marshaller
 * and unmarshaller by setting properties, schemas, adapters, and listeners, and
 * to refer to it.
 *
...
public class JaxWsJaxb2Marshaller implements MimeMarshaller, MimeUnmarshaller, GenericMarshaller, GenericUnmarshaller,
	 ...
	 * "classesToBeBound"} or {@link #setPackagesToScan "packagesToScan"} is
	 * required.
	 */
	...
	/**
	 * Set the packages to search for classes with JAXB2 annotations in the
	 * classpath. This is using a Spring-bases search and therefore analogous to
	...
	/**
	 * Set the JAXB {@code Marshaller} properties. These properties will be set
	 * on the underlying JAXB {@code Marshaller}, and allow for features such as
	 * indentation.
	 * 
	 * @param properties
	 *            the properties
	...
	/**
	 * Set the JAXB {@code Unmarshaller} properties. These properties will be
	 * set on the underlying JAXB {@code Unmarshaller}.
	 * 
	 * @param properties
	 *            the properties
	...
	/**
	 * Set the JAXB validation event handler. This event handler will be called
	 * by JAXB if any validation errors are encountered during calls to any of
	 * the marshal APIs.
	 */
	...
	/**
	 * Specify the {@code XmlAdapter}s to be registered with the JAXB
	...
	/**
	 * Set whether to lazily initialize the {@link JAXBContext} for this
	 * marshaller. Default is {@code false} to initialize on startup; can be
	 * switched to {@code true}.
	 * <p>
	 * Early initialization just applies if {@link #afterPropertiesSet()} is
	 * called.
	 */
	 ...
	 * {@code false}: marshalling using XOP/MTOM not being enabled.
	 */
	 ...
	 * <p>
	 * Default is {@code false}, meaning that {@code supports(Class)} always
	 * returns {@code false} for {@code JAXBElement} classes (though
	 * {@link #supports(Type)} can return {@code true}, since it can obtain the
	 * type parameters of {@code JAXBElement}).
	 * <p>
	 * This property is typically enabled in combination with usage of classes
	 * like {@link org.springframework.web.servlet.view.xml.MarshallingView
	 * MarshallingView}, since the {@code ModelAndView} does not offer type
	 ...
	 * <p>
	 * Default is {@code true}, meaning that {@code supports(Class)} will check
	 * for this annotation. However, some JAXB implementations (i.e. EclipseLink
	 * MOXy) allow for defining the bindings in an external definition file,
	 * thus keeping the classes annotations free. Setting this property to
	 * {@code false} supports these JAXB implementations.
	 * 
	 * @see #supports(Class)
	 * @see #supports(Type)
	 */
	 ...
	 * {@link #unmarshal(Source)} is a {@link SAXSource} or
	 * {@link StreamSource}. It has no effect for {@link DOMSource} or
	 * {@link StAXSource} instances.
	 */
	...
	/**
	 * Returns the configured value for whether XML external entities are
	 * allowed.
	 */
	...
	/**
	 * Return the JAXBContext used by this marshaller, lazily building it if
	 * necessary.
	 */
	...
	private JAXBContext createJaxbContextFromContextPath() throws JAXBException {
		...
		if (this.jaxbContextProperties != null) {
			if (this.beanClassLoader != null) {
				...
				// analogous to the JAXBContext.newInstance(String)
				// implementation
	...
	/**
	 * Checks whether the given type is a primitive wrapper type. Compare
	 * section 8.5.1 of the JAXB2 spec.
	 */
	...
	/**
	 * Checks whether the given type is a standard class. Compare section 8.5.2
	 * of the JAXB2 spec.
	 */
	private boolean isStandardClass(Class<?> clazz) {
		return String.class.equals(clazz) || BigInteger.class.isAssignableFrom(clazz)
				|| BigDecimal.class.isAssignableFrom(clazz) || Calendar.class.isAssignableFrom(clazz)
				|| Date.class.isAssignableFrom(clazz) || QName.class.isAssignableFrom(clazz) || URI.class.equals(clazz)
				|| XMLGregorianCalendar.class.isAssignableFrom(clazz) || Duration.class.isAssignableFrom(clazz)
				|| Image.class.equals(clazz) || DataHandler.class.equals(clazz) ||
				// Source and subclasses should be supported according to the
				// JAXB2 spec, but aren't in the RI
	...
	/**
	 * Return a newly created JAXB marshaller. JAXB marshallers are not
	...
	/**
	 * Template method that can be overridden by concrete JAXB marshallers for
	 * custom initialization behavior. Gets called after creation of JAXB
	 * {@code Marshaller}, and after the respective properties have been set.
	 * <p>
	 * The default implementation sets the {@link #setMarshallerProperties(Map)
	 * defined properties}, the
	 * {@link #setValidationEventHandler(ValidationEventHandler) validation
	 * event handler}, the {@link #setSchemas(Resource[]) schemas},
	 * {@link #setMarshallerListener(javax.xml.bind.Marshaller.Listener)
	 * listener}, and {@link #setAdapters(XmlAdapter[]) adapters}.
	 */
	...
	/**
	 * Return a newly created JAXB unmarshaller. Note: JAXB unmarshallers are
	 ...
	 * {@link #setUnmarshallerProperties(Map) defined properties}, the
	 * {@link #setValidationEventHandler(ValidationEventHandler) validation
	 * event handler}, the {@link #setSchemas(Resource[]) schemas},
	 * {@link #setUnmarshallerListener(javax.xml.bind.Unmarshaller.Listener)
	 * listener}, and {@link #setAdapters(XmlAdapter[]) adapters}.
	 */
	...
	/**
	 * Convert the given {@code JAXBException} to an appropriate exception from
	 * the {@code org.springframework.oxm} hierarchy.
	 * 
	 * @param ex
	 *            {@code JAXBException} that occured
