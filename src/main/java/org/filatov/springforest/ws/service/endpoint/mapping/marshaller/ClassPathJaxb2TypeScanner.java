...
/**
 * Helper class for {@link Jaxb2Marshaller} that scans given packages for
 * classes marked with JAXB2 annotations.
 *
			...
			for (String packageToScan : this.packagesToScan) {
				...
				for (Resource resource : resources) {
					...
					if (isJaxb2Class(metadataReader, metadataReaderFactory)) {
						...
						Class<?> jaxb2AnnotatedClass = this.resourcePatternResolver.getClassLoader()
								.loadClass(className);
						if (null != jaxb2AnnotatedClass.getAnnotation(XmlRootElement.class)) {
							jaxb2Classes.add(jaxb2AnnotatedClass);
						}
						// int modifiers = jaxb2AnnotatedClass.getModifiers();
